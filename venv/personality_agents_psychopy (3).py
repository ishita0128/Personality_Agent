...
def get_experiment_info():
    dlg_info = {"Personality Agent": list(PERSONALITIES.keys())}
    dlg = gui.DlgFromDict(dictionary=dlg_info, title="Select Chatbot Agent",
                          order=["Personality Agent"])
    if dlg.OK:
        return dlg_info
    core.quit()
...
class ConversationalAgent:
    ...
    def respond(self, user_input=""):
     ...
     client = Groq(api_key="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")
...
def save_chat_csv(chat_log, participant_id="P001"):
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"chat_{participant_id}_{timestamp}.csv"
    
    with open(filename, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        # Header row
        writer.writerow(["turn", "role", "message", "timestamp"])
        
        for i, (role, text) in enumerate(chat_log):
            writer.writerow([i+1, role, text, timestamp])
...
def main():
    ...
    chat_log = run_conversation(win, agent)
    save_chat_csv(chat_log, participant_id="P001")
    win.close()
    core.quit()
